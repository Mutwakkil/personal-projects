...
let followUp = 0, quizQuestion, scores = 0;
...
function renderOptions () {
  let html = '', optionA, optionB, optionC, optionD;
  ...
  function handleOptionClicksRemove () {
    ...
    document.querySelector('.js-next-button').addEventListener('click', handleNextClick);
    ...
    function handleNextClick() {
      ...
      if (followUp === 9) {
        document.querySelector('.js-next-button').removeEventListener('click', handleNextClick);
      }
    }
    ...
    document.querySelector('.js-submit-button').addEventListener('click', () => {
      ...
      document.querySelector('.js-scores').innerHTML = `Total score is ${scores}/10`
      handleOptionClicksRemove();
      document.querySelector('.js-next-button').removeEventListener('click', handleNextClick);
      if (weldone.textContent === 'Correct!' || weldone.textContent === 'Wrong!') {
        weldone.style.visibility = 'hidden';
      }
      document.querySelectorAll('.js-container, .js-buttons-container').forEach((element) => {
    ...
    document.querySelector('.js-start-button').addEventListener('click', () => {
      document.querySelectorAll('.js-start-button-container, .js-start-button').forEach((button) => {
        button.style.display = 'none';
      });
      document.querySelectorAll('.js-next-button, .js-submit-button, .js-scores').forEach((element) => {
    ...
    document.querySelector('.js-no-button').addEventListener('click', () => {
      ...
      document.querySelectorAll('.js-container, .js-buttons-container').forEach((button) => {
